...
static void do_nlist (CHAR_DATA *ch, char *argument)
{
	...
	for (p = ch->pcdata->board->note_first; p ; p = p->next)
	{
		...
		if (is_note_to(ch,p))
		{
			...
			{
				xprintf (buf, "#w%3d"  "#0>"  "#R%c#n"  "#w%-13s#n"  " #w%s" " \n\r",
				               num, 
...
void do_board (CHAR_DATA *ch, char *argument)
{
  ...
  if (IS_NPC(ch)) return;
	...
	if (!argument[0]) /* show boards */
	{
		...
		count = 1;
		send_to_char("\n\r", ch);
    send_to_char("#WNum #0Name        #wNew#R/#wAll  #WDescription#n\n\r", ch);
    send_to_char("#w=== ========== ========= ===========#n\n\r", ch);
		for (i = 0; i < MAX_BOARD; i++)
		{
    ...
    if (ch->pcdata->board != NULL)
    {
      xprintf(buf, "\n\r#wYour current board is #W%s#n.\n\r", ch->pcdata->board->short_name);
		...
		if (ch->pcdata->board->read_level > get_trust(ch))
			send_to_char ("#0You cannot read nor write notes on this board.#n\n\r",ch);
		else if (ch->pcdata->board->write_level > get_trust(ch))
			send_to_char ("#0You can only read notes from this board.#n\n\r",ch);
		else
			send_to_char ("#0You can both read and write on this board.#n\n\r",ch);
		}
		send_to_char("\n\r",ch);
		return;			
	} /* if empty argument */
	...
	if (is_number(argument))
	{
		...
		if (count == number) /* found the board.. change to it */
		{
			...
			xprintf (buf, "Current board changed to " BOLD "%s" NO_COLOR ". %s.\n\r",boards[i].short_name,
