...
void song_update(void)
{
        ...
        if (channel_songs[1] > -1)
        {
                ...
                else
                {
                        ...
                        {
                                victim = d->original ? d->original : d->
                                        character;

                                if (d->connected == CON_PLAYING &&
                                    !IS_SET(victim->act, PLR_NOMUSIC) &&
                                    !IS_SET(victim->act, PLR_NOMUSIC))
                                        act("$t", d->character, buf, NULL,
                                            TO_CHAR);
                        }
                }
        }
        ...
        for (obj = object_list; obj != NULL; obj = obj->next)
        {
                ...
                else
                {
                        ...
                        line = song_table[obj->value[1]].lyrics[obj->
                                                                value[0]];
                        obj->value[0]++;
                }

                xprintf(buf, "$p bops: '{N%s{x'", line);
                if (obj->in_room->people != NULL)
...
void do_play(CHAR_DATA * ch, char *argument)
{
        ...
        if (!str_cmp(arg, "list"))
        {
                ...
                xprintf(buf, "%s has the following songs available:\n\r",
                        juke->short_descr);
                        ...
                        if (artist && (!match
                                ...
                                xprintf(buf, "%-39s %-39s\n\r",
                                 ...
                                 && (!match
                                     || !str_prefix(argument,
                                                    song_table[i].name)))
                                xprintf(buf, "%-35s ", song_table[i].name);
                        else
                                continue;
                        add_buf(buffer, buf);
                        if (!artist && ++col % 2 == 0)
